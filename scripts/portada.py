...
TITULO = "OMNI Logistics"
LEMA = "De la cámara del almacén a saber cuánto queda libre ahora"
FRASE = ("Un detector de fuego a secas marca cada foco del almacén. La comprobación de brillo es lo que hace que la alarma se atienda.")
...
def main() -> int:
    med = medidas()
    y11 = med.get("pallet_n_640.pt", {})
    fps = (f"{1000 / y11['mejor_ms']:.0f} fps"
           if y11.get("mejor_ms") else "—")
    mapa = ((y11.get("metricas") or {}).get("mAP50"))
    acierto = f"{mapa * 100:.0f} %" if mapa else "—"

    PASOS = [
        ("01", "video", "Entra", "Las cámaras que ya hay",
         f"0 €", f"en sensores nuevos"),
        ("02", "caja", "Detecta", "Cuatro modelos, uno por trabajo",
         f"{acierto}", f"mAP@50 en pallets"),
        ("03", "fuego", "Vigila", "Fuego, humo y EPP",
         f"1", f"alarma, no cien"),
        ("04", "grafico", "Mide", "Ocupación, muelle y rutas",
         f"4", f"módulos en un tablero"),
    ]
    ...
    n = len(med)
    for i, (et, val) in enumerate((("Pruebas", f"8 pasando"), ("Modelos medidos", f"{n}"), ("Corre en", f"CPU o GPU"), ("Datos simulados", f"ninguno"))):
        x = m + i * ((W - 2 * m) / 4)
    ...
    print(f"  docs/portada.svg  {len(svg) // 1024} KB · {fps} · {acierto}")
    return 0
